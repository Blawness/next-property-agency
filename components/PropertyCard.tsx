...
import { MapPin, BedDouble, Bath, Maximize2, MessageCircle } from "lucide-react"
import type { PropertyWithImages } from "@/lib/types"
import { formatPriceCompactValue, SITE_URL } from "@/lib/constants"
...
export default function PropertyCard({
  ...
  return (
    <div className="group relative">
      ...
      {whatsappHref && (
        /* Sits just inside the bottom edge of the h-56 (14rem) photo. */
        <a
          href={whatsappHref}
          target="_blank"
          rel="noopener noreferrer"
          aria-label={`Tanya lewat WhatsApp: ${property.title}`}
          className="absolute right-3 top-44 z-20 inline-flex items-center gap-1.5 rounded-sm bg-accent px-3 py-2 text-[12px] font-semibold text-accent-foreground shadow-sm transition-colors hover:bg-primary"
        >
